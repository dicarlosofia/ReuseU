  ...
  const handleDelete = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          setListings(
            listings.filter((listing: any) => listing.ListingID !== ListingID)
          );
        }, 300);
      }, 2000);
    } catch (error) {
      ...
      >
        ...
        <div className="w-full md:w-1/4 aspect-square bg-lime-100 rounded-lg flex-shrink-0">
          {image && (
            <img
              src={image}
              alt={title}
              className="text-cyan-300 w-full h-full object-cover rounded-lg"
            />
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 justify-between">
          <div>
            ...
            <p className="text-cyan-800 text-xs md:text-sm line-clamp-2 mt-1">
              {desc}
            </p>
          </div>
        ...
        <div className="w-20 flex flex-col items-end justify-between max-w-full overflow-hidden">
          <div
            className="text-gray-700 text-sm mb-2 truncate-description"
            style={{
              display: "-webkit-box",
              WebkitLineClamp: 2,
            ...
            {reportSuccess ? (
              ...
              <form onSubmit={handleReportSubmit} className="space-y-3">
                <label className="block">
                  <span className="text-cyan-900 text-sm font-medium">
                ...
                {reportError && (
                  <div className="text-red-600 text-xs">{reportError}</div>
                )}
                <button
                  type="submit"
